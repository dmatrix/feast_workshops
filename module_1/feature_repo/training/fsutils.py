...
def create_entity_df(drivers: list[int]) -> pd.DataFrame:
    """
    Create an entity dataframe to be joined with Entity rows from the data source
    """
...
def get_online_data(repo_path: str, id: int) -> dict:
    """
    Fetch feature vector for for inferencing
    """
    store=fst.FeatureStore(repo_path=repo_path)
